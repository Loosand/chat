...
Chat 是一个从简开始的多模型聊天平台 Monorepo。Goal 1 聊天领域/持久化，以及 Goal 2 的生产认证及其最薄界面、模型目录、AI adapter、chat run 执行器、HTTP/刷新恢复和单模型环境 bootstrap 已完成；当前正在实现最薄聊天前端。完整模型网关、计费、工具与后台仍处于后续 Goal，不得把目标设计写成已实现能力。
...
- `packages/network-security` 是 server-only 共享 SSRF/DNS rebinding 防线，不承载模型、MCP、文件或存储业务规则。
