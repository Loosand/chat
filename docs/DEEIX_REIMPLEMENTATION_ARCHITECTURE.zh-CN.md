...
| UI | React 19、Tailwind、Base Rhea、Base UI、shadcn Chat primitives、Streamdown | 统一无样式行为层、流式 Markdown 与仓库自有源码 | UI 与 domain 分离；不直接依赖 Radix-only AI Elements |
...
| ADR-015 | UI primitive 使用 shadcn/ui Base Rhea + Base UI；聊天外壳采用 shadcn Chat primitives，文本采用 Streamdown，AI parts 自有组合 |
