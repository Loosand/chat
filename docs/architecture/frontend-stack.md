...
> 状态：前端基础依赖与通用 primitive 已实现；聊天页面、UIMessage part registry、传输、持久化和恢复仍为规划。
> 审计日期：2026-08-12。
...
| AI UI state | AI SDK 7 + `@ai-sdk/react` 4 `useChat` | 依赖已安装，未接线 |
| Markdown | Streamdown 2 + CJK、Math、Code、Mermaid plugins | 最小封装已实现，未接线 |
| 流协议 | AI SDK UIMessage SSE | 规划 |
| 表单 contract | Zod 4；复杂表单再引入 React Hook Form | Zod 已实现，表单规划 |
...
| Part | 渲染责任 |
| --- | --- |
| `text` | `StreamingMarkdown`；只渲染 assistant 生成文本 |
| `reasoning` | 可折叠的 reasoning summary；不假定供应商会暴露完整思维链 |
| `tool-*` | 按工具名和 state 渲染 input streaming、待批准、执行、结果和错误 |
| `source-url` | 结构化来源列表和外链确认，不依赖模型手写脚注解析 |
...
- Client 使用 `useChat<ChatUIMessage>` 和 `DefaultChatTransport`。
- Web Route Handler 返回 `createUIMessageStreamResponse(toUIMessageStream(...))`。
- Client 只提交最后一条用户消息、conversation id、run id 和当前模型选择；Server 重新加载并校验历史。
- Server 使用 `validateUIMessages` 校验 metadata、data parts 和 tool schemas。
- `originalMessages`、稳定 server message id 和 `onEnd` 用于合并与持久化。
- UIMessage 是版本化的 UI/wire snapshot，不替代领域 message、run、usage 和 billing facts。
...
| Profile | 刷新行为 |
| --- | --- |
| Vercel Core / Docker Lite | Server 加载已持久化消息；未持久化的进行中 run 可显示为 interrupted |
| Vercel Full / Docker Full | `useChat({ resume: true })` + Redis `resumable-stream` + resume endpoint |

可恢复 stream 把刷新、断网和路由跳转视为 disconnect。显式“停止生成”必须调用独立 stop endpoint，持久 partial assistant snapshot、取消 producer 并清除 active stream。客户端 `stop()` 只停止本地连接，不代替服务端取消，也不要求 Trigger.dev。
